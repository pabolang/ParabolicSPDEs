...
#'
#' Simulate Monte-Carlo samples of a SPDE model in one space dimension on a discrete grid with \code{N} temporal and \code{M} spatial points, where the grid points are equidistant within the unit square. Each reputation uses the "replacement"-method by default, see [ParabolicSPDEs::simulateSPDEmodel].
#' @param reputations A natural number giving the number of the Monte-Carlo repetitions.
#' @param ... further arguments see [ParabolicSPDEs::simulateSPDEmodel].
...
#' l <- MCSPDESamples(reputations,theta0,theta1,theta2,sigma,numberSpatialPoints,numberTemporalPoints)


MCSPDESamples <- function(reputations, theta0,theta1,theta2,
                                    sigma,numberSpatialPoints,numberTemporalPoints,L=10,xi=function(x){0*x},cutoff = 10000,method = "replacement"){

  MC_list <- lapply(1:reputations, function(i){
